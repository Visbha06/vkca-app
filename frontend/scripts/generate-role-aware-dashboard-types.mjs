...
const generatedPath = path.join(
  frontendDirectory,
  'src/features/dashboard/api/generated.ts',
)
const checkOnly = process.argv.includes('--check')
const temporaryDirectory = mkdtempSync(
  path.join(tmpdir(), 'vkca-role-aware-dashboard-openapi-'),
)
...
try {
  run(
    ...
    [
      'run',
      'python',
      'scripts/export_role_aware_dashboard_openapi.py',
      '--output',
      openapiPath,
    ],
    backendDirectory,
  ...
  if (checkOnly) {
    ...
    if (committed !== candidate) {
      console.error(
        'Role-aware dashboard API types have drifted. Run npm run generate:role-aware-dashboard-types.',
      )
      process.exitCode = 1
    } else {
      console.log('Role-aware dashboard API types are current.')
    }
  }
} finally {
