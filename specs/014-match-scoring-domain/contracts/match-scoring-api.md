...
The real request schema is strict: it rejects unknown fields, missing required fields, duplicate orders, invalid side combinations, an innings sequence that does not match the selected capability, one-day limits or six-ball over length that are missing or not 300/60/6, player IDs outside the configured academy team, account fields on external participants, unsupported reserved dismissal identifiers, and client-supplied derived score fields. `format`, `policy_code`, and `capability_profile` must use the exact canonical values above; `t20`, `one_day`, and `other_manual` are rejected with 422. The capability profile derives allowed completion modes, result codes, dismissal types, transition types, and the explicit Match-completion boundary; callers cannot supply contradictory capability fields or code sets. Policy-supplied `other` values are required before scoring, while the initial T20, one-day, and test values are fixed by the capability version. The list above is illustrative and must contain the configured XI.
